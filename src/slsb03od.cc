...
DEFUN_DLD (slsb03od, args, nargout, "Slicot SB03OD Release 5.0")
{
    ...
    else
    {
        ...
        if (f77_exception_encountered)
            error ("lyap: slsb03od: exception in SLICOT subroutine SB03OD");

        if (info != 0)
            error ("lyap: slsb03od: SB03OD returned info = %d", info);
